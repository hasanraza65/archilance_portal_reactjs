...
const determineCategory = (appName, windowTitle) => {
  ...
  const socialKeywords = ["facebook", "instagram", "youtube", "whatsapp", "twitter", "tiktok", "netflix", "spotify", "discord", "reddit"];
  if (socialKeywords.some((k) => lowerName.includes(k) || lowerTitle.includes(k))) return "Social";

  const productiveKeywords = [
    "visual studio", "vscode", "pycharm", "git", "github", "docker", "postman", 
    "excel", "word", "powerpoint", "outlook", "teams", "slack", "zoom", "meet", 
    "archilance", "autocad", "revit", "photoshop", "figma", "canva", "chatgpt", "claude", "ai studio"
  ];
  if (productiveKeywords.some((k) => lowerName.includes(k) || lowerTitle.includes(k))) return "Productive";

  if (lowerName.includes("chrome") || lowerName.includes("edge") || lowerName.includes("firefox")) {
    if (lowerTitle.includes("admin") || lowerTitle.includes("crm") || lowerTitle.includes("archilance") || lowerTitle.includes("docs")) return "Productive";
    return "Neutral";
  }
  return "Neutral";
};
...
const calculateTooltipPos = (data) => {
  ...
  // Outer Radius (90) + Gap (15) = 105px from center
        ...
        if (activity?.app_name && activity?.duration_seconds) {
          ...
          if (!isNaN(dur) && dur > 0) {
            ...
            if (!appMap[cleanAppName]) {
              appMap[cleanAppName] = { duration: 0, category: category, count: 0 };
            }
    ...
    <div className="font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-white dark:bg-slate-800 p-5 rounded-lg shadow-sm border border-slate-100 dark:border-slate-700">
          <p className="text-xs text-slate-500 dark:text-slate-400 mb-1 font-semibold uppercase tracking-wider">Total Time</p>
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">{formatDuration(dashboardData.stats.totalSeconds)}</h2>
        </div>
        <div className="bg-white dark:bg-slate-800 p-5 rounded-lg shadow-sm border border-slate-100 dark:border-slate-700">
          <p className="text-xs text-green-500 dark:text-green-400 mb-1 font-semibold uppercase tracking-wider">Productive Time</p>
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">{formatDuration(dashboardData.stats.productiveSeconds)}</h2>
        </div>
        <div className="bg-white dark:bg-slate-800 p-5 rounded-lg shadow-sm border border-slate-100 dark:border-slate-700">
          <p className="text-xs text-amber-500 dark:text-amber-400 mb-1 font-semibold uppercase tracking-wider">Idle Time</p>
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">{formatDuration(dashboardData.stats.idleSeconds)}</h2>
        </div>
        <div className="bg-white dark:bg-slate-800 p-5 rounded-lg shadow-sm border border-slate-100 dark:border-slate-700">
          <p className="text-xs text-indigo-500 dark:text-indigo-400 mb-1 font-semibold uppercase tracking-wider">Productivity %</p>
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">
            {dashboardData.stats.totalSeconds > 0
              ? Math.round((dashboardData.stats.productiveSeconds / dashboardData.stats.totalSeconds) * 100)
              : 0}%
          </h2>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div key={i} className="group">
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2 overflow-hidden">
                       <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: i < 5 ? APP_COLORS[i % APP_COLORS.length] : '#94a3b8' }}></div>
                       <span className="text-sm font-medium text-slate-700 dark:text-slate-200 truncate max-w-[150px]" title={app.name}>{app.name}</span>
                    </div>
                    <span className="text-xs text-slate-500 font-mono">{formatDuration(app.duration)}</span>
                  </div>
                  <div className="w-full bg-slate-100 dark:bg-slate-700 rounded-full h-1.5 overflow-hidden">
                    <div 
                      className="h-1.5 rounded-full" 
                      style={{ 
                        width: `${(app.duration / dashboardData.stats.totalSeconds) * 100}%`,
                        backgroundColor: i < 5 ? APP_COLORS[i % APP_COLORS.length] : '#94a3b8'
                      }}
                    ></div>
                  </div>
                </div>
        ...
        {/* RIGHT: Donut Chart with External Tooltip */}
                        ...
                        >
                          {dashboardData.pieData.map((entry, index) => (
                            <Cell 
                              ...
                              opacity={hoveredSlice && hoveredSlice.name !== entry.name ? 0.6 : 1}
                            />
                          ))}
                        </Pie>
                        {/* Standard Tooltip Disabled */}
                      </PieChart>
                    </ResponsiveContainer>

                    {/* CUSTOM ABSOLUTE TOOLTIP (Outside Circle) */}
                    {hoveredSlice && (
                        <div 
                            ...
                            style={{
                                ...
                                // Dynamic transformation to keep it visible
                                transform: `translate(${hoveredSlice.midAngle < 180 ? '10px' : '-110%'}, -50%)`,
                                minWidth: '140px'
                            }}
                        >
                            <div className="flex items-center gap-2 mb-1 border-b border-slate-700 pb-1">
                                <div className="w-2 h-2 rounded-full" style={{ backgroundColor: hoveredSlice.payload.color }}></div>
                                <span className="font-bold text-xs truncate max-w-[120px]">{hoveredSlice.name}</span>
                            </div>
                            <div className="flex justify-between items-center text-xs">
                                <span className="text-slate-300 font-mono">{formatDuration(hoveredSlice.value)}</span>
                                <span className="font-bold text-emerald-400">
                                    {((hoveredSlice.value / hoveredSlice.payload.total) * 100).toFixed(1)}%
                                </span>
                            </div>
                        </div>
                    )}
                ...
                {dashboardData.pieData.length > 0 && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-0">
                    <span className="text-2xl font-bold text-slate-800 dark:text-white">
                      {formatDuration(dashboardData.stats.totalSeconds).split(' ')[0]} 
                    </span>
                    <span className="text-[10px] uppercase text-slate-400 tracking-wider">Total Time</span>
                  </div>
                )}
            </div>
                    ...
                    {dashboardData.pieData.map((entry, index) => (
                        <div 
                            ...
                            className={`flex items-center justify-between p-2 rounded transition-all duration-200
                                ${hoveredSlice && hoveredSlice.name === entry.name ? 'bg-slate-100 dark:bg-slate-700 scale-[1.02]' : 'hover:bg-slate-50 dark:hover:bg-slate-700/50'}
                            `}
                        >
                            <div className="flex items-center gap-3">
                                <div className="w-3 h-3 rounded-full shadow-sm" style={{ backgroundColor: entry.color }}></div>
                                <span className="text-sm font-medium text-slate-700 dark:text-slate-200 truncate max-w-[120px] sm:max-w-[150px]" title={entry.name}>
                                    {entry.name}
                                </span>
                            </div>
                            <div className="flex items-center gap-4">
                                <span className="text-xs font-bold text-slate-800 dark:text-slate-100 w-10 text-right">
                                    {((entry.value / dashboardData.stats.totalSeconds) * 100).toFixed(1)}%
                                </span>
