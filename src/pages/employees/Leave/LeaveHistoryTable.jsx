...
import { Calendar, Filter, Search, Clock, CheckCircle, XCircle, AlertCircle } from 'lucide-react';
...
const LeaveHistoryTable = ({ leaves = [] }) => {
  ...
  // Sample data if no leaves provided
  const sampleLeaves = [
    {
      id: 1,
  ...
  // Status configuration with icons and enhanced styling
  const statusConfig = {
    Approved: {
      icon: CheckCircle,
  ...
  const filteredLeaves = displayLeaves
    .filter(leave => {
  ...
  const getStatusStats = () => {
    const stats = displayLeaves.reduce((acc, leave) => {
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-white/20">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{displayLeaves.length}</p>
                </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredLeaves.length > 0 ? (
                  filteredLeaves.map((leave, index) => {
                    ...
                    const duration = leave.days || calculateDuration(leave.startDate, leave.endDate);
                      ...
                      >
                        ...
                        <td className="px-6 py-6">
                          <div>
                            ...
                            {leave.type && (
                              <span className={`px-3 py-1 text-xs font-medium rounded-full ${leaveTypeColors[leave.type] || 'bg-gray-100 text-gray-800'}`}>
                                {leave.type}
                              </span>
                            )}
                          </div>
                        ...
                        <td className="px-6 py-6">
                          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl font-medium text-sm ${statusConfig[leave.status]?.className || 'bg-gray-100 text-gray-800'}`}>
                            <div className={`w-2 h-2 rounded-full ${statusConfig[leave.status]?.dotColor || 'bg-gray-400'}`}></div>
