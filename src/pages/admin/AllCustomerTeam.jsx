...
const AllCustomerTeam = () => {
  ...
  const fetchAllCustomerTeams = useCallback(async (currentPage) => {
    ...
    }/api/admin/customer-team?page=${currentPage}`;
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message || "Failed to fetch customer teams.";
    ...
    (teamMember) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          const deleteUrl = `${
            import.meta.env.VITE_BACKEND_BASE_URL
          }/api/admin/customer-team/${teamMember.id}`;
        ...
        Cell: ({ row }) => (
          <div className="flex space-x-3 rtl:space-x-reverse">
            <Tooltip content="Delete" placement="top" arrow>
              <button
                className="p-1.5 text-red-600 hover:text-red-800 hover:bg-red-100 dark:text-red-400 dark:hover:text-red-300 dark:hover:bg-red-900/50 rounded-md"
    ...
    return (
      <Card>
        <div className="p-4 text-center text-danger-500">
          Error: {fetchError} <br />
          <button
            onClick={() => fetchAllCustomerTeams(1)}
      ...
      <Card noBorder>
        ...
        <div className="md:flex md:space-y-0 space-y-5 justify-between mt-6 items-center">
          <div className="text-sm text-slate-600 dark:text-slate-300">
            Showing {apiMetadata.from || 0} to {apiMetadata.to || 0} of{" "}
            {apiMetadata.total || 0} entries
          </div>
          <ul className="flex items-center space-x-2 rtl:space-x-reverse">
            <li>
              <button
                ...
                disabled={page === 1}
                className="text-slate-600 dark:text-slate-300 p-2 hover:bg-slate-100 dark:hover:bg-slate-600 rounded"
              >
                <Icon icon="heroicons-outline:chevron-left" />
              </button>
            </li>
            {Array.from({ length: apiMetadata.last_page }, (_, i) => i + 1).map(
              (pageNum) => (
                <li key={pageNum}>
                  <button
                    ...
                    } hover:bg-slate-200 dark:hover:bg-slate-600 rounded`}
            ...
            <li>
              <button
                onClick={() =>
                  setPage((p) => Math.min(apiMetadata.last_page, p + 1))
                }
                disabled={page === apiMetadata.last_page}
                className="text-slate-600 dark:text-slate-300 p-2 hover:bg-slate-100 dark:hover:bg-slate-600 rounded"
              >
