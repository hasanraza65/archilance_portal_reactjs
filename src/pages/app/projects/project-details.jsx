...
// ... (mapApiAssigneeToLocal, getStatusClass, getPriorityClass functions remain the same) ...
const mapApiAssigneeToLocal = (apiAssignee) => {
  ...
  const name =
    apiAssignee.name ||
    (apiAssignee.user && apiAssignee.user.name) ||
    "Unknown";
...
const ProjectDetailsPage = () => {
  ...
  const handleDeleteTask = async (taskId) => {
    if (!taskId) {
      ...
      Swal.fire({
        // SweetAlert for missing task ID
        icon: "error",
        title: "Oops...",
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (!token) {
          setError("Authorization token not found. Please log in.");
          Swal.fire({
            // SweetAlert for token error during delete
            icon: "error",
            title: "Authentication Error",
            text: "Authorization token not found. Please log in.",
          }).then(() => {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            fetchProjectAndTasks(); // Re-fetch to ensure UI consistency
            return;
          }
          ...
          Swal.fire({
            // SweetAlert for network/other error during deletion
            icon: "error",
            title: "Error",
  ...
  if (error && !projectDetails && !projectFound) {
    // Initial load critical error, error state might already show a Swal from fetch.
    // This div is a fallback.
    return (
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {error &&
        projectDetails && ( // Non-critical error (e.g., failed re-fetch)
      ...
      {projectDetails && projectFound ? (
        tasks.length > 0 ? (
          <div className="bg-white dark:bg-gray-700 rounded-lg shadow overflow-hidden">
            ...
            <div className="grid grid-cols-12 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-600 text-sm font-medium text-gray-500 dark:text-gray-300 sticky top-0 z-10">
              ...
              {tasks.map((task, index) => {
                const assignee =
                  task.assignees && task.assignees.length > 0
                    ? mapApiAssigneeToLocal(task.assignees[0])
                    : null;
                return (
                  <div
                    key={task.id || `task-${index}`}
                    className="grid grid-cols-12 border-b border-gray-200 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors "
                  >
                    <div
                      className="col-span-4 p-4 flex items-center cursor-pointer"
                    ...
                    <div
                      className="col-span-2 p-4 flex items-center cursor-pointer"
                      onClick={() => navigate(`/task/${task.id}`)}
                    >
                      {assignee ? (
                        <div className="flex items-center">
                          <span
                            className={`w-6 h-6 ${assignee.color} text-white rounded-full flex items-center justify-center text-xs font-medium mr-2`}
                          >
                            {assignee.avatar}
                          </span>
                    ...
                    <div
                      className="col-span-1 p-4 flex items-center cursor-pointer"
                      onClick={() => navigate(`/task/${task.id}`)}
                    >
                      <span
