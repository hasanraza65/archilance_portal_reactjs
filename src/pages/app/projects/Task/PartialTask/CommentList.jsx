...
const buildCommentTree = (commentsList) => {
  ...
  const tree = [];
  commentsList.forEach((c) => {
    const node = treeNodesMap[c.id];
    if (c.reply_to && treeNodesMap[c.reply_to]) {
      if (c.id !== c.reply_to) {
        const parentNode = treeNodesMap[c.reply_to];
      ...
      const sortGrandchildren = (children) => {
        children.forEach((child) => {
          if (child.children.length > 0) {
            ...
            sortGrandchildren(child.children);
          }
        });
      };
...
const CommentList = ({
  ...
  const handleStartReply = (commentToReplyTo) => {
    const sender = commentToReplyTo.sender
      ? mapApiUserToLocal(commentToReplyTo.sender, "commenter")
  ...
  const handleKeyDownOnNewComment = (e) => {
    if (e.key === "Enter" && !e.shiftKey) {
      ...
      if (newCommentFormRef.current) {
        const noContent =
          (!newComment || !newComment.trim()) && attachments.length === 0;
        const busy =
          isSubmittingComment ||
          isProcessingEditOrDelete ||
        ...
        if (noContent || busy) {
          ...
          else if (newComment.length > 5000)
  ...
  const RenderCommentNode = ({ commentNode, level }) => {
    const sender = commentNode.sender
      ? mapApiUserToLocal(commentNode.sender, "commenter")
      : mapApiUserToLocal(null, "commenter");
              ...
              {isEditingThis ? (
                <div className="bg-slate-100 rounded-xl p-4 border border-blue-300 shadow-md">
                  ...
                  <div className="flex items-center justify-end space-x-2">
                    ...
                    >
                      {isProcessingEditOrDelete ? "Saving..." : "Save Changes"}
                    </button>
                  </div>
                </div>
              ) : (
