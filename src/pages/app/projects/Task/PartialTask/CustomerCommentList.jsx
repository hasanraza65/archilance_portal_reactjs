...
const DropdownMenu = ({ options, onSelect, onClose }) => {
  ...
  return (
    <div ref={dropdownRef} className="absolute z-50 mt-1 w-32 bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
      <div className="py-1">
        {options.map((option) => (
          <button
            key={option.label}
...
const CustomerCommentList = ({ taskId, currentUserId, handleCommentSubmit, onEditComment, onDeleteComment }) => {
  ...
  const fetchComments = useCallback(async (urlOverride = null, silent = false, source = "unknown") => {
    ...
    try {
      ...
      setNewPageUrl(apiResponse.next_page_url || apiResponse.links?.next || null);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (!isFetchingRef.current && !isRecording && !isSubmittingComment && !document.hidden) {
  ...
  const handleWrapperDelete = async (id) => {
     const res = await Swal.fire({ title: "Are you sure?", text: "This message will be deleted.", icon: "warning", showCancelButton: true, confirmButtonText: "Yes" });
     if (res.isConfirmed) {
  ...
  const handleWrapperEdit = async (commentId, formData) => {
      ...
      if (success) { setEditingCommentId(null); fetchComments(); }
      setIsProcessingEditOrDelete(false);
  }
  ...
  const RenderCommentNode = ({ commentNode }) => {
      ...
      return (
        <div className="group relative py-2 pl-3 sm:pl-4">
             <div className="flex items-start space-x-3" onContextMenu={(e) => {
                 e.preventDefault();
                 setDropdownPosition({ top: e.clientY, left: e.clientX });
                 setActiveDropdownCommentId(commentNode.id);
             }}>
                {sender.profilePic ? (
                  <img src={sender.profilePic} className="w-9 h-9 rounded-full object-cover ring-1 ring-slate-100" />
                ) : (
                    ...
                    {isEditingThis ? (
                        <div className="bg-slate-100 rounded-xl p-4 border border-blue-300 shadow-md">
                            ...
                            <div className="flex justify-end gap-2">
                                <button onClick={()=>setEditingCommentId(null)} className="px-3 py-1.5 text-sm bg-slate-200 rounded-lg">Cancel</button>
                        ...
                        <div className={`rounded-xl p-3.5 group-hover:bg-slate-100 transition-colors ${getBubbleColor(commentNode)}`}>
                             ...
                             {commentNode.comment_attachments?.map(att => {
                                 const path = att.file_path.startsWith('http') ? att.file_path : `${STORAGE_BASE_PATH}${att.file_path}`;
                                 return (
                                     <div key={att.id} className="mt-1">
                                         {isAudio(att) ? <AudioPlayer src={path} /> : (
                                             <a href={path} target="_blank" rel="noreferrer" className="text-xs text-blue-600 underline block truncate">📎 {att.file_name}</a>
                                         )}
                                     </div>
                                 );
                             })}
             ...
             {activeDropdownCommentId === commentNode.id && (
                <div style={{ position: "fixed", top: dropdownPosition.top, left: dropdownPosition.left, zIndex: 100 }}>
                    <DropdownMenu
                        options={[{ label: "Edit", action: "edit", disabled: sender.id !== currentUserId }, { label: "Delete", action: "delete", disabled: sender.id !== currentUserId && userRole !== 'admin' }]}
                        onSelect={(act) => {
            ...
            {commentNode.replies?.map(r => <div key={r.id} className="ml-5 sm:ml-7 border-l-2 border-slate-200/70 pl-3 sm:pl-4 mt-2"><RenderCommentNode commentNode={r} /></div>)}
        </div>
      );
  };
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 mt-8">
      ...
      <div ref={commentsListRef} className="p-6 space-y-2 h-[400px] overflow-y-auto custom-scrollbar bg-slate-50/30">
          <div ref={loadMoreRef} className="h-10 flex justify-center items-center text-xs text-slate-400">
             {isLoading ? "Loading messages..." : newPageUrl ? "Scroll up for chat history" : "Conversation started"}
          </div>
      ...
      <div className="p-6 border-t border-slate-200 bg-slate-50/50 rounded-b-2xl">
          {replyingToComment && (
              <div className="mb-3 p-2.5 bg-indigo-50 rounded-lg text-sm relative border border-indigo-100 flex justify-between items-center">
                  ...
                  <button onClick={() => setReplyingToComment(null)} className="text-xl leading-none">×</button>
              </div>
          )}
              ...
              {isRecording ? (
                ...
                {attachments.length > 0 && (
                    <div className="flex flex-wrap gap-2 p-2 bg-white rounded-lg border border-slate-200 mb-2">
                        {attachments.map(att => (
                            <div key={att.id} className="flex items-center gap-2 bg-slate-100 px-2 py-1 rounded-md">
                ...
                <div className="flex items-end space-x-3">
                    <label className="flex-shrink-0 p-2.5 border border-slate-300 rounded-xl cursor-pointer bg-white hover:bg-slate-50 transition-all">
                        <svg className="w-6 h-6 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" /></svg>
                    ...
                    {newComment.trim() === "" && attachments.length === 0 ? (
                        <button type="button" onClick={handleStartRecording} className="bg-emerald-500 text-white w-12 h-12 rounded-full flex items-center justify-center shadow-lg"><FontAwesomeIcon icon={faMicrophone} className="text-xl" /></button>
                    ) : (
                        <button type="submit" disabled={isSubmittingComment} className="bg-indigo-600 text-white w-12 h-12 rounded-full flex items-center justify-center shadow-lg">
                            {isSubmittingComment ? "..." : <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg>}
                        </button>
                    )}
                </div>
                </>
