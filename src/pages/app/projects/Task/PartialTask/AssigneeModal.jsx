...
const AssigneeModal = ({
  ...
  useEffect(() => {
    // Initialize selected IDs when modal opens or current assignees change
    if (isOpen) {
      setSelectedEmployeeIds(new Set(currentAssigneeIds.map(id => String(id)))); // Ensure IDs are strings for Set comparison
    }
  ...
  const handleSave = () => {
    // Pass array of numbers if your backend expects numbers
    onSaveAssignees(taskId, Array.from(selectedEmployeeIds).map(id => parseInt(id, 10)));
  };

  if (!isOpen) return null;

  const mappedEmployees = allEmployees.map(emp => mapApiUserToLocal(emp)).filter(Boolean);
  ...
  return (
    <div className="fixed inset-0 bg-slate-100 bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-[9999] p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md max-h-[80vh] flex flex-col">
        ...
        <div className="p-1 space-y-1 overflow-y-auto flex-grow">
          {mappedEmployees.length > 0 ? (
            ...
            <p className="text-slate-500 text-center py-6">No employees available or failed to load.</p>
          )}
        </div>
...
// src/pages/projects/ProjectDetailsPage.jsx (or wherever your file is)
