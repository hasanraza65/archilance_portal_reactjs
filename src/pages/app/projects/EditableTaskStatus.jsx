...
const EditableTaskStatus = ({
  ...
  const handleSelectOption = async (newStatus) => {
    ...
    try {
      ...
      const url = `${import.meta.env.VITE_BACKEND_BASE_URL}/api/${apiPath}/project-task/${taskId}`;
            ...
            >
              {STATUS_OPTIONS.map((option) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2.5 text-sm hover:bg-slate-100 dark:hover:bg-slate-700 flex items-center relative ${displayStatus === option ? "font-semibold text-blue-600 dark:text-blue-400" : "text-slate-700 dark:text-slate-300"}`}
                >
                  {displayStatus === option && (<span className={`absolute left-0 top-0 bottom-0 w-1 ${getStatusSelectedBarColor(option)}`}></span>)}
