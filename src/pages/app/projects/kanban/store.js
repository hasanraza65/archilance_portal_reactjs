...
export const fetchKanbanData = createAsyncThunk(
  ...
  async (projectId, { rejectWithValue }) => {
    ...
    }/api/${apiPrefix}/project-task?project_id=${projectId}`;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message ||
            `Failed to fetch Kanban data (${response.status})`
        );
      }
      ...
      if (Array.isArray(tasksToProcess) && tasksToProcess.length > 0) {
        const sortedTasks = tasksToProcess.sort(
          (a, b) => (a.board_order || 0) - (b.board_order || 0)
        );
        ...
        console.log(
          "fetchKanbanData: No tasks returned from API for project",
          projectId
        );
      } else {
        console.warn(
          "fetchKanbanData: tasksToProcess is not an array or is undefined.",
          apiResponse
        );
      }
      ...
      return rejectWithValue(
        error.message || "Unknown error fetching Kanban data"
      );
    }
  }
);
...
export const updateTaskStatusInBackend = createAsyncThunk(
  ...
  async ({ taskId, taskData, newStatus }, { rejectWithValue, getState }) => {
    ...
    const apiUrl = `${
      import.meta.env.VITE_BACKEND_BASE_URL
    }/api/${apiPrefix}/project-task/${taskId}`;
    ...
    if (!fullTaskData.project_id && taskId) {
      ...
      for (const column of projectColumns) {
        foundTask = column.tasks.find(
          (t) => t.apiData && String(t.apiData.id) === String(taskId)
        );
        if (foundTask) break;
      }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorData = {
            message: `HTTP error ${response.status}. Non-JSON: ${responseBodyText}`,
          };
        }
        const displayError =
          errorData.message ||
      ...
      if (
        ...
        return rejectWithValue(
          "Incomplete data from server after task status update."
        );
      }
      ...
      return {
        ...
        newStatus,
        updatedTaskDataFromApi: updatedTaskFromApi.task,
      };
    } catch (error) {
...
export const updateTaskOrderInBackend = createAsyncThunk(
  ...
  async ({ taskId, boardOrder }, { rejectWithValue }) => {
    ...
    const apiUrl = `${
      import.meta.env.VITE_BACKEND_BASE_URL
    }/api/${apiPrefix}/change-project-task-order`;
...
export const deleteTaskFromBackend = createAsyncThunk(
  ...
  async ({ backendTaskId, frontendTaskId }, { rejectWithValue }) => {
    ...
    const apiUrl = `${
      import.meta.env.VITE_BACKEND_BASE_URL
    }/api/${apiPrefix}/project-task/${backendTaskId}`;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const displayError =
          ...
          (errorData.errors &&
            Object.values(errorData.errors).flat().join(", ")) ||
          `Failed to delete task (Server error ${response.status})`;
...
export const appKanbanSlice = createSlice({
  ...
  reducers: {
      ...
      if (type === "task") {
        ...
        if (
          ...
          if (newStatusKey) {
            ...
            if (targetColIndex !== -1) {
              const taskInNewColIndex = state.columns[
                targetColIndex
              ].tasks.findIndex((t) => String(t.id) === String(removedTask.id));
              if (
                taskInNewColIndex !== -1 &&
                ...
                state.columns[targetColIndex].tasks[taskInNewColIndex].apiData =
                  {
                    ...state.columns[targetColIndex].tasks[taskInNewColIndex]
                      .apiData,
    ...
    deleteColumnBoard: (state, action) => {
      ...
      toast.warn("Board Deleted Successfully (Frontend Only)", {
        autoClose: 1500,
      });
    },
    toggleTaskModal: (state, action) => {
      if (typeof action.payload === "object" && action.payload !== null) {
        ...
        if (action.payload.taskData && action.payload.mode === "edit") {
          console.warn(
            "toggleTaskModal with mode 'edit' called. External EditTaskModal should be used."
          );
        }
    ...
    addTask: (state, action) => {
      const column = state.columns.find(
        (col) => String(col.id) === String(state.openTaskId)
      );
      if (column) {
        const taskStatusKey =
        ...
        const newTask = {
          ...
          apiData: {
            ...
            assignees: (action.payload.assignee || []).map((a) =>
              typeof a === "object" && a.id !== undefined ? a.id : a
            ),
            categories: (action.payload.category || []).map((c) =>
              typeof c === "object" && c.id !== undefined ? c.id : c
            ),
          },
        };
        column.tasks.push(newTask);
        toast.success("Task Added (Frontend Only - Implement Backend Save)", {
          autoClose: 1500,
        });
      }
    ...
    toggleEditModal: (state, action) => {
      console.warn(
        "toggleEditModal reducer called. This may be deprecated if using external EditTaskModal exclusively."
      );
    },
    updateTask: (state, action) => {
      console.warn(
        "updateTask reducer called. Ensure this is still needed with external EditTaskModal handling backend updates and KanbanPage re-fetching."
      );
      ...
      state.columns = state.columns.map((column) => {
        const taskIndex = column.tasks.findIndex(
          (task) =>
            String(task.id) === String(updatedTaskPayload.id) ||
            (task.apiData &&
              String(task.apiData.id) === String(updatedTaskPayload.id))
        );
        if (taskIndex !== -1) {
          const existingTask = column.tasks[taskIndex];
          const newName =
            updatedTaskPayload.name !== undefined
              ? updatedTaskPayload.name
              : existingTask.name;
      ...
      if (taskFoundAndUpdated) {
        toast.info("Task Updated (Frontend Only via updateTask reducer)", {
          autoClose: 1500,
        });
      }
    },
    ...
    builder
      ...
      .addCase(updateTaskStatusInBackend.fulfilled, (state, action) => {
        ...
        state.columns = state.columns.map((column) => {
          const taskIndex = column.tasks.findIndex(
            (task) =>
              task.apiData && String(task.apiData.id) === String(backendTaskId)
          );
          if (taskIndex !== -1) {
            ...
            tasksCopy[taskIndex] = {
              ...
              progress:
                updatedTaskDataFromApi.progress !== undefined
                  ? updatedTaskDataFromApi.progress
                  : oldTask.progress,
              assignee:
                updatedTaskDataFromApi.assignees !== undefined
                  ? updatedTaskDataFromApi.assignees
                  : oldTask.assignee,
              category:
                updatedTaskDataFromApi.categories !== undefined
                  ? updatedTaskDataFromApi.categories
                  : oldTask.category,
      ...
      .addCase(updateTaskStatusInBackend.rejected, (state, action) => {
        console.error(
          "updateTaskStatusInBackend.rejected:",
          action.payload,
      ...
      .addCase(updateTaskOrderInBackend.rejected, (state, action) => {
        console.error(
          "updateTaskOrderInBackend.rejected:",
          action.payload,
          `For Task API ID: ${action.meta.arg.taskId}`
        );
      })
      ...
      .addCase(deleteTaskFromBackend.rejected, (state, action) => {
        console.error(
          "deleteTaskFromBackend.rejected:",
          action.payload,
