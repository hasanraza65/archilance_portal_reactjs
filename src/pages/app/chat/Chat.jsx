    ...
    <div className="h-full">
      ...
      <div className="chat-content parent-height">
        <div className="msgs overflow-y-auto msg-height pt-6 space-y-6" ref={chatheight}>
          ...
          {!isMessagesLoading && !messagesError && messFeed.map((item) => (<div className="block md:px-6 px-4" key={item.id}><div className={`flex space-x-2 items-start group rtl:space-x-reverse ${item.sender === 'me' ? 'justify-end' : ''}`}>{item.sender === "them" && (<div className="flex-none"><UserAvatar avatarUrl={item.img} fullName={item.senderFullName} className="h-8 w-8" /></div>)}<div className="flex-1 flex space-x-4 rtl:space-x-reverse max-w-[70%]"><div className={`p-3 rounded-xl text-sm break-words ${item.sender === 'me' ? 'bg-slate-900 text-white rounded-tr-none' : 'bg-slate-100 dark:bg-slate-600 dark:text-slate-300 text-slate-600 rounded-tl-none'}`}>{item.content}<div className={`text-xs mt-1 ${item.sender === 'me' ? 'text-slate-300' : 'text-slate-400'}`}>{item.time}</div></div></div>{item.sender === "me" && (<div className="flex-none"><UserAvatar avatarUrl={loggedInUser?.profile_pic} fullName={loggedInUser?.name} className="h-8 w-8" /></div>)}</div></div>))}
        </div>
      </div>
