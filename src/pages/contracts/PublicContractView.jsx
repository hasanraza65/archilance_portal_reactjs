...
import { fetchPublicContract, acceptPublicContract } from "./contractsApi";
...
const PublicContractView = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (active) setError(err.response?.data?.message || "This contract link is invalid or has expired.");
      } finally {
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "We couldn't record your acceptance. Please try again.");
      setAccepting(false);
    }
  };
      ...
      <header className="sticky top-0 z-10 bg-white/90 backdrop-blur border-b border-slate-200">
        <div className="max-w-3xl mx-auto px-5 py-3.5 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <span className="w-9 h-9 rounded-lg bg-indigo-600 text-white flex items-center justify-center font-bold">
              A
            </span>
            ...
            <div className="text-center py-2">
              ...
              <p className="text-sm text-slate-500 mt-1">
                {contract?.accepted_at
                  ? `Accepted on ${formatDateTime(contract.accepted_at)}. `
                  : ""}
                Please check your email for your login details to the Archilance Portal and Time Tracker.
              </p>
            </div>
          ) : (
            <div>
              <p className="text-center text-xs text-slate-500 mb-3">
                By sliding below, you confirm that you have read and agree to the terms of this
