...
const AddCardModal = ({ isOpen, onClose, onAddCard }) => {
  ...
  const handleInputChange = (e) => {
    ...
    if (
      ...
      (cardDetails[name] === "John Doe" ||
        cardDetails[name] === "0000 0000 0000 0000" ||
        cardDetails[name] === "MM/YY" ||
        cardDetails[name] === "123")
    ) {
  ...
  const inputClasses =
    "block w-full px-4 py-3 bg-gray-100 rounded-lg text-gray-500 border border-transparent focus:outline-none focus:ring-2 focus:ring-gray-300 focus:bg-white focus:text-gray-900";
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md relative transition-transform transform-gpu scale-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="pt-4">
            <button
              type="submit"
              className="w-full text-center py-3 bg-gray-800 text-white rounded-lg text-base font-semibold hover:bg-gray-900 transition-all shadow-md hover:shadow-lg"
            >
              Add Card
            </button>
          </div>
