...
const pricingData = [
  {
    ...
    features: [
      ...
      "1 team member dedicated to your project",
    ],
    isPopular: false,
  },
  {
    ...
    features: [
      ...
      "Time Tracking",
      "2 team members dedicated to your project",
    ],
    ...
    >
      ...
      {isPopular && !isCurrentPlan && (
        <div className="absolute top-0 -translate-y-1/2 w-full flex justify-center">
          <span className="bg-gray-800 text-white text-xs font-semibold px-4 py-1.5 rounded-full uppercase">
            Most Popular
          </span>
        </div>
      )}
      {isCurrentPlan && (
        <div className="absolute top-0 -translate-y-1/2 w-full flex justify-center">
          <span className="bg-blue-600 text-white text-xs font-semibold px-4 py-1.5 rounded-full uppercase">
            Current Plan
          </span>
        </div>
      )}
...
const UpgradePlan = () => {
  ...
  useEffect(() => {
    ...
    const defaultPlan =
      pricingData.find((p) => p.name === currentPlan) ||
      pricingData.find((p) => p.isPopular);
    if (defaultPlan) {
        ...
        {/* <div className="flex justify-center mt-8">
          <div className="relative bg-gray-200 p-1 rounded-full flex items-center">
            <button
              ...
              className={`w-full px-8 py-2 text-sm font-semibold rounded-full relative z-10 transition-colors duration-300 ${
                !isAnnual ? "text-gray-800" : "text-gray-500"
              }`}
            >
              Monthly
            </button>
