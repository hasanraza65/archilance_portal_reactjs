...
const UpdateCustomer = () => {
  ...
  const fetchCustomerData = useCallback(async () => {
    ...
    try {
      ...
      if (response.data && typeof response.data === 'object' && response.data.id) {
        ...
        setFetchError("Failed to load customer data or data is in unexpected format.");
      }
    } catch (err) {
      console.error("Error fetching customer data:", err);
      setFetchError(err.response?.data?.message || err.message || "Failed to load customer data. An unknown error occurred.");
    } finally {
      setLoading(false);
    }
  }, [customerId, reset]);
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      ...
      if (err.response?.data?.errors) {
        ...
        setSubmitError(err.response?.data?.message || err.message || "Failed to update customer. An unknown error occurred.");
      }
    } finally {
      setSubmitting(false);
    }
  ...
  if (loading) {
    return <Card title="Loading Customer Data..."><div className="p-6 text-center text-slate-600 dark:text-slate-300">Please wait...</div></Card>;
  }
  if (fetchError) {
    return (
      <Card title="Error">
        <div className="p-6">
          <Alert icon="heroicons-outline:exclamation-triangle" className="alert-danger light-mode">{fetchError}</Alert>
          <Button text="Go Back" className="btn-dark mt-4" onClick={() => navigate(-1)} />
        </div>
      </Card>
    );
  }
